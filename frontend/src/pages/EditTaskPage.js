...
class EditTaskPage extends React.Component {
    ...
    toggleContributor(user_id,user_name){
        var currentContributors = this.state.contributors;
        var cont = this.isContributor(user_id);
        if(!cont){
            this.removeContributor(user_id);
        } else {
            this.addContributor(user_id, user_name)
        }
    }

    getContributor = (user_id) => {
        var newList = this.state.contributors;
    ...
    addContributor = (user_id, user_name) => {
        var newList = this.state.contributors;
    ...
    removeTag(e,id) {
        ...
        const newList = this.state.taskTags.filter((tag) => tag !== id);
        this.setState({taskTags:newList});
    ...
    componentDidMount() {
        ...
        if (this.state.task_id){
            ...
            task.getTags().then(data => this.setState({availableTags: data.tags }));
        }
        ...
        return (
            <div>
                ...
                <div className="homeContainer"> 
                    ...
                    <div className="tagsBOX"> 
                                ...
                                <Dropdown >
                                    ...
                                    {this.state.availableTags.map(tag => {
                                        return (
                                            <div>
                                                <Dropdown.Item  onSelect={(s) => this.addTag(tag)}> 
                                                {tag}
                                                </Dropdown.Item>
                                            </div>
                                        )
                                ...
                                {this.state.isOpen && <Popup
                                    ...
                                    handleClose={(s) => this.togglePopup(s)}
                                    />}
                    </div>
                    <div> 
                    ...
                    <div>
...

                                {
                                    this.state.users.map(user => {
                                        return (
                                            <div >
                                                <button  className="userContainer" onClick={(e) => this.toggleContributor(user.user_id, user.name)}>
                                                    
                                                    <div className="dateText"><b>{user.name}</b></div>
                                               
                                                <CheckboxSelected className={this.isContributor(user.user_id) ? "hide" : "show"}></CheckboxSelected>
                                                
                                                <Checkbox className={this.isContributor(user.user_id) ? "show" : "hide"}> </Checkbox>
                                                </button>
                                    ...
                                    <div className="listDate">
                                        <div className="dateContainer">
                                            ...
                                            <div className="dateContainer">
                                                ...
                                                <Dropdown>
                                                    <Dropdown.Toggle className="userDrop" variant="secondary btn-sm" id="dropdown-basic">
                    ...
                    <Link className="link" to={{pathname:"/task", task_id:this.state.task_id, user_id:this.state.user_id}}>
